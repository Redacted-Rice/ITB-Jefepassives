...
local function isRanged(pawn)
	local ranged = {
		--"Burnbug", --Gastropod --> moved to isProjectile
		"Moth",
		"Totem", --Spore
		"DNT_Ladybug", --Vextra
		"DNT_Silkworm", --Vextra
...
--or hook!
local function isProjectile(pawn)
	local proj = {
		"Firefly1",
		"Firefly2",
		--Firely leader is DIFFERENT... Or maybe it's actually okay?
		"FireflyBoss",

		"Burnbug1",
		"Burnbug2",
		"BurnBugBoss",
		--So is Burnbug leader? Or not, it just put flames around

		"Snowtank1",
		"Snowtank2",
	}

	return list_contains(proj, pawn:GetType())
end

--TODO
local function isLaser(pawn)
	local las = {
		"Snowlaser1",
		"Snowlaser2",
	}

	return list_contains(las, pawn:GetType())
end
...
--NEW
local function canTargetProjectile(pawn, target, mustBeAbleToHit)
	if pawn == nil or target == nil then
		if SHOW_LOGS then LOG("D - canTargetProjectile -> nil -> return false") end
		return false
	end
	if isAligned(pawn:GetSpace(), target) then
		--return not mustBeAbleToHit or target == GetProjectileEnd(pawn:GetSpace(), target)
		if not mustBeAbleToHit then
			if SHOW_LOGS then LOG("D - canTargetProjectile -> no need to actually see the target -> return true") end
			return true
		else
			local projEnd = GetProjectileEnd(pawn:GetSpace(), target)
			if SHOW_LOGS then LOG("D - canTargetProjectile -> projEnd: "..projEnd:GetString()..", target: "..target:GetString()) end
			return target == projEnd
		end
	end
	if SHOW_LOGS then LOG("D - canTargetProjectile -> not aligned (or other reasons) -> return false") end
	return false
end

--NEW
local function canTargetLaser(pawn, target, mustBeAbleToHit)
...
--New: added a parameter (mustBeAbleToHit) where the enemy needsto be able to actually hit the target. (projectiles may be blocked)
function taunt.canTargetNewPoint(pawn, target, mustBeAbleToHit)
	if SHOW_LOGS then LOG("C - taunt.canTargetNewPoint(pawn: "..pawn:GetType().." ("..pawn:GetSpace():GetString().."), target: "..target:GetString()..", mustBeAbleToHit: "..tostring(mustBeAbleToHit)..")") end

	--test
	if not isAligned(pawn:GetSpace(), target) then --though some weird attacks won't need to be aligned
		if SHOW_LOGS then LOG("   ---------> NOT ALIGNED -> RETURN FALSE") end
		return false
	end

	--Hornets and modded vek exceptions
	local name = pawn:GetType()
	if name == "Firefly1" or name == "Firefly2" then
		if SHOW_LOGS then LOG("canTargetNewPoint -> Firefly") end
		--TODO: Firefly leader, with its dual projectiles
		--canTargetProjectile(pawn, target, mustBeAbleToHit)
		return canTargetProjectile(pawn, target, mustBeAbleToHit)
	elseif name == "Hornet2" or name == "lmn_Chomper1" or name == "lmn_Chomper2" or name == "lmn_Chili1" or name == "lmn_Chili2" then --Hornet and chompers with 2 range and chilis
		if SHOW_LOGS then LOG("canTargetNewPoint -> RANGE == 2 (Hornet2, lmn_Chomper1, ...)") end
	...
	elseif name == "HornetBoss" or name == "lmn_KnightBot" or name == "lmn_ChomperBoss" or name == "lmn_ChiliBoss" or isVek(pawn, "DNT_IceCrawler") then --Hornet Leader + Knight Bots with 3 range + Chomper Boss with 3 range + chili boss with 3 range + Ice Crawlers with 3 range
		if SHOW_LOGS then LOG("canTargetNewPoint -> RANGE == 3 (HornetBoss, lmn_KnightBot, ...)") end
	...
	elseif isVek(pawn, "lmn_Springseed") then --Springseed with essentially a melee attack, but its target area is two
		if SHOW_LOGS then LOG("canTargetNewPoint -> lmn_Springseed") end
		return canTargetSpringseed(target, pawn)
	elseif name == "DNT_DragonflyBoss" then --Dragonfly Boss has 3 range arty
		if SHOW_LOGS then LOG("canTargetNewPoint -> DNT_DragonflyBoss") end
	...
	elseif name == "lmn_Cactus1" or name == "lmn_Cactus2" or name == "lmn_SpringseedBoss" or isVek(pawn,"DNT_Mantis") then		
		if SHOW_LOGS then LOG("canTargetNewPoint -> range == 2 (lmn_Cactus1, lmn_Cactus2, ...)") end
	...
	if isRanged(pawn) then
		if SHOW_LOGS then LOG("canTargetNewPoint -> Other ranged") end
		local id = pawn:GetId()
	...
	if SHOW_LOGS then LOG("canTargetNewPoint -> All Else") end
	--All Else
	local queuedWeaponName = pawn:GetQueuedWeapon()
    if queuedWeaponName == nil then
    	if SHOW_LOGS then LOG("queuedWeaponName == nil -> return false") end
        return false
    end
    ...
    for _, v in pairs(extract_table(areaPoints)) do
        if v == target then
        	if SHOW_LOGS then LOG("-----------------------> return true") end
            return true
        end
    end

    if SHOW_LOGS then LOG("canTargetNewPoint - Did not find a corresponding point -> return false") end

    --Did not find a corresponding point
    return false
end

local function test_canTargetNewPoint(pawn, target, mustBeAbleToHit)
	if SHOW_LOGS then LOG("B - test_canTargetNewPoint(mustBeAbleToHit: "..tostring(mustBeAbleToHit)..")") end
	return taunt.canTargetNewPoint(pawn, target, mustBeAbleToHit)
end


--Checks if the enemy can be taunted by the given point
--New: is "public"
function taunt.canBeTauntedByPoint(pawn, point, mustBeAbleToHit)
	if SHOW_LOGS then LOG("A - taunt.canBeTauntedByPoint(mustBeAbleToHit: "..tostring(mustBeAbleToHit)..")") end
	if pawn == nil or point == nil then
		if SHOW_LOGS then LOG("pawn == nil or point == nil -> RETURN FALSE") end
		return false
	end
	local id = pawn:GetId()
	local space = pawn:GetSpace()
	...
	--return taunt.canTargetNewPoint(pawn, point, mustBeAbleToHit)-- and not target == point and not target == Point(-1,-1)
	--return self.canTargetNewPoint(pawn, point, mustBeAbleToHit)
	return test_canTargetNewPoint(pawn, point, mustBeAbleToHit)
end
...
function taunt.enemy(id, point)
	...
	if Board:IsTipImage() or pawn == nil or not pawn:IsEnemy() or not taunt.canBeTauntedByPoint(pawn, point) then return end
	...
	if isRanged(pawn) then --Ranged
		...
		if dist > 1 then --Artillery Vek
			...
			if dist > 5 or dist == 1 then
				if SHOW_LOGS then LOG("Out of the artillery's range") end
				return --Out of the artillery's range
			end
		end
...
function taunt.addTauntEffectEnemy(effect, id, point, dmg, failFlag)
	...
	if Board == nil or Board:IsTipImage() then return end

	local pawn = Board:GetPawn(id)

	if pawn == nil or not pawn:IsEnemy() then return end
	...
	if taunt.canBeTauntedByPoint(pawn, point) then
