...
Jefepassives_RandomSwap_Passive = PassiveSkill:new{
	--Infos
	...
	--Passive
	Passive = "Jefepassives_RandomSwap_Passive",
...
function Jefepassives_RandomSwap_Passive:GetSkillEffect(p1, p2)
...
local function swap(pawnId1, pawnId2)
	...
	--Maybe board alert will add annoying delays
	--Board:AddAlert(p1, "EXPERIMENTAL SWAP")
	--Board:AddAlert(p2, "EXPERIMENTAL SWAP")
end

local EVENT_onNextTurn = function(mission)
	--LOG("EVENT_onNextTurn - Random Swap")

	if Game:GetTeamTurn() == TEAM_PLAYER and IsPassiveSkill("Jefepassives_RandomSwap_Passive") then
		local enemies = {}
