...
Place the `Jefepassives` folder in `Into the Breach/mods/`. When you open the folder you should see the `scripts` directory directly inside it.
...
| Migratory Invoker | Das Keifer | `Jefepassives_MigratoryInvoker` | At the start of the Vek turn, each Vek attempts to move one tile to the right. |
| Experimental Swapper | Truelch | `Jefepassives_RandomSwap_Passive` | At the start of player's turn, swap two random enemies. |
| RST Decoy | Das Keifer | `Jefepassives_RstDecoy` | At mission start setups a decoy on a random tile. |
